...
export const registrarFrequencia = async (req, res) => {
    try {
        ...
        const resultado = await frequenciaModel.criarFrequencia(aluno_id, data, presente);
...
export const atualizarFrequencia = async (req, res) => {
    try {
        ...
        const resultado = await frequenciaModel.atualizarFrequencia(id, aluno_id, data, presente);

        if (resultado.affectedRows === 0) {
            return res.status(404).json({ msg: "Registro não encontrado" });
        }

        res.json({ msg: "Frequência atualizada" });
    } catch (error) {
        res.status(500).json({ erro: error.message });
    }
};
